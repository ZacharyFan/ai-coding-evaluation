...
The benchmark task asks the workflow to fix `catalog.ValidateProduct` so it rejects blank or whitespace-only product SKUs while preserving valid active catalog products.
...
python scripts/score_run.py \
  --task benchmarks/tasks/go-bugfix-l1-c1/task.json \
  --run runs/baseline/go-bugfix-l1-c1/demo-002/run.json \
  --score runs/baseline/go-bugfix-l1-c1/demo-002/score.json \
  --set-review \
    correctness=1.0 \
