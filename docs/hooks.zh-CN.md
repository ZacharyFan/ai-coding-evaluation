...
可以把 `integrations/claude-code/settings.example.json` 作为 Claude Code settings 的模板。Claude Code settings 可以放在：
...
把模板复制或合并到其中一个 settings 文件。benchmark run 场景下，优先放到 target worktree 的 `.claude/settings.local.json`，这样 hook 只作用于当前 run，不会污染所有 Claude Code 项目。
