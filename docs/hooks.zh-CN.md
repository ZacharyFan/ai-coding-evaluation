...
`python -m scripts.eval hooks` 会把 Codex 和 Claude Code hook 文件写入准备好的 target worktree，并加入该 worktree 的本地 `.git/info/exclude`。

已被 target repo 跟踪的 hook 文件永远不会被修改。

如果未被跟踪的 hook 文件已经存在，运行：

```bash
python -m scripts.eval hooks --merge
```

它会保留已有 hook，并追加评估 recorder hook，且不会重复追加同一条命令。只有明确想覆盖未被跟踪的 generated hook 文件时才使用 `--force`。
...
`python -m scripts.eval hooks` 会把 `integrations/codex/config.example.toml` 和 `integrations/codex/hooks.example.json` 安装到当前 target worktree。

使用 `--merge` 时，它会保留已有未跟踪的 Codex hooks，确保 `features.codex_hooks = true`，并对每个事件只追加一次评估 recorder hook。

hook command 会使用 `$AI_EVAL_REPO`，所以启动 Codex 前必须先执行 `eval "$(python -m scripts.eval env)"`；如果不在仓库根目录，就用 `bin/ai-eval env`。
...
benchmark run 场景下，优先放到 target worktree 的 `.claude/settings.local.json`，这样 hook 只作用于当前 run，不会污染所有 Claude Code 项目。

如果已有未跟踪的项目级 Claude settings 文件，使用：

```bash
python -m scripts.eval hooks --agent claude --merge
```

它会保留已有 hook，并对每个事件只追加一次评估 recorder hook。不要把 secret 写进可提交文件。
