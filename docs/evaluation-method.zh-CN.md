...
`python -m scripts.prepare_run` 创建这组证据骨架，把 coding task prompt 复制到 run 目录，并把目标仓库 clone 到隔离的 run worktree。`acceptance.md` 不会被复制，因为它是 reviewer-only 文件。`python -m scripts.collect_run` 采集目标项目的 setup/test 结果，写入 `test.log`，写入 `diff.patch`，并更新 `run.json` 中机械测试和 diff 事实。可选 hooks 会把过程事件追加到 `events.jsonl`；`python -m scripts.summarize_run_events` 会从这些事件派生过程证据和 coding workflow 墙钟 `duration_minutes`。没有 hooks 时，`duration_minutes` 是人工维护的 run 事实。人工 reviewer 通过 `python -m scripts.score_run --set-review` 传入 `review.*` 分数，也可以用 `python -m scripts.llm_review_run` 调用 OpenAI-compatible LLM 生成 review 维度。`python -m scripts.score_run` 计算最终评分字段，并写入 `derived_hard_gates` 和最终合并后的 `hard_gates`。

当 `task.json` 定义 `scope.allowed_paths` 时，`python -m scripts.collect_run` 会把已跟踪变更文件和 untracked 新文件一起与 allowlist 对比，并写入 `run.json.diff.scope_check=path_allowlist`、`unrelated_files_changed` 和 `unrelated_files`。如果没有 `scope`，无关文件状态是未知，而不是默认干净。
...
行级采纳率来自真实 git refs，不来自 `diff.patch`：`base_ref -> candidate_ref` 表示 AI candidate 变更，`base_ref -> accepted_ref` 表示最终采纳变更。两个 ref 都存在后，用 `python -m scripts.adoption_lines` 写入 `run.json`。如果没有行级采纳数据，但有 run 级 `adoption.adoption_rate`，脚本会取这些 run 的平均值。如果没有采纳数据，采纳率保持为空。它不会用 `score / 100` 冒充采纳率。
