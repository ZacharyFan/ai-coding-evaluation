...
3. Replace `task.md`, `acceptance.md`, and `tests.sh` with a concrete task from a real repository.
4. Validate locally:
...
```text
task.json       Machine-readable metadata, budget, target repo, checks, and scoring weights
...
`benchmarks/tasks/` is the real benchmark set. Templates live under `benchmarks/templates/` and are not run by default.
...
- A fixed target repo and `base_ref`
...
- The task can be run from a clean checkout of the target repo
- The scoring weights still sum to 100
